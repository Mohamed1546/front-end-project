...
@section('content')
    <div class="row">
        <div class="col-10 mx-auto">
            ...
            <form action="{{ route('admin.sections.update',$section->id) }}" method="post" class="border p-4" enctype="multipart/form-data">
                @csrf
                @method("put")
                <div class="row mb-4">
                    ...
                    <div class="col-12 mb-4">
                        ...
                        <select name="user_id" id="user_id1" class="form-control SlectBox">
                            ...
                            <option value="{{ $user->id }}"@if ($section->user_id==$user->id) selected @endif> {{ $user->firstname.' '.$user->lastname }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-12 mb-4">
                        ...
                        <select class="form-control" name="profile_id" aria-label="Default select example">
                            @if ($section->profile_id!=null)
                                <option value="{{$section->profile_id}}" selected>{{$section->profile->name}}</option>
                            @endif
                        </select>
                    </div>
                    <div class="col-12 mb-4">
                ...
                <input type="submit" name="submit" value="{{__('admin.edit_sections')}}" class="btn btn-primary">
            </form>
        </div>
    </div>
@endsection
